...
class DocumentProcessor:
    """
    Backend class to handle all document processing operations
    """
    ...
    def extract_content_from_pdf(self, pdf_path):
        """Extract content using the elsai_core library"""
    ...
    def extract_content_from_pdf_direct(self, pdf_path):
        """
    ...
    def _format_table(self, tables):
        """
        Format the table into a string representation.
        
        Args:
            tables: The table objects to format
        Returns:
    ...
    def _convert_to_markdown(self, text_content, tables):
        """
        ...
        if tables:
            ...
            for i, table in enumerate(tables):
                ...
                markdown_parts.append(self._format_table_as_markdown(table))
                markdown_parts.append("\n\n")
        
        logger.debug("Markdown conversion complete")
        return "".join(markdown_parts)

    def extract_content_from_csv(self, csv_path):
    ...
    def process_file(self, file_path, document_type):
        ...
        try:
            ...
            if file_path.lower().endswith('.pdf'):
                ...
                content = self.extract_content_from_csv(file_path)
            else:
    ...
    def _get_prompt_name_by_type(self, document_type):
        """
    ...
    def process_pdf_advanced(self, uploaded_file, document_type):
        """
        ...
        try:
            ...
            text_content, tables = self.extract_content_from_pdf_direct(tmp_path)
            
            # Convert to markdown
            logger.info("Converting extracted content to markdown")
            markdown_content = self._convert_to_markdown(text_content, tables)
            ...
            prompt_name = self._get_prompt_name_by_type(document_type)
    ...
    def generate_comparison_summary(self, invoice_content, po_content):
        """Generate a comparison summary between invoice and PO content"""
        try:
            connector = AzureOpenAIConnector()
            llm = connector.connect_azure_open_ai(deploymentname="gpt-4o-mini")
            renderer = PezzoPromptRenderer(
            ...
            prompt_txt = f"{prompt}\n\nText: {invoice_content},{po_content}"
            response = llm.invoke(prompt_txt)
            return response.content
        except Exception as e:
            logger.error(f"Error generating comparison summary: {str(e)}", exc_info=True)
            return f"Error generating comparison: {str(e)}"

    def process_invoice_pdf(self, file_path):
        """
        ...
        try:
            # Extract content from PDF
            text_content, tables = self.extract_content_from_pdf(file_path)
            tables_str = self._format_table(tables)
